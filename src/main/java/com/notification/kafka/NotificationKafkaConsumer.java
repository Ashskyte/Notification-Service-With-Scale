...
import com.notification.service.DeduplicationService;
...
public class NotificationKafkaConsumer {
    ...
    public void consumeSendEvent(NotificationEvent event) {
        ...
        // Deduplication: skip if this event was already processed (Kafka at-least-once redelivery)
    ...
    public void consumeRetryEvent(NotificationEvent event) {
        log.info("Consumed RETRY event for notification [{}] from topic [notification-retry]",
                event.getNotificationId());
        // Deduplication: skip if this retry event was already processed
        if (deduplicationService.isDuplicate("retry:" + event.getNotificationId())) {
            log.warn("Duplicate RETRY event for notification [{}] - skipping", event.getNotificationId());
            return;
        }
        Notification notification = notificationRepository.findByIdWithUser(event.getNotificationId())
                .orElseThrow(() -> {
